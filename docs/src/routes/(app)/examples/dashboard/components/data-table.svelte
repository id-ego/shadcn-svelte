	...
	export const columns: ColumnDef<Schema>[] = [
		...
		},
		{
			...
			header: () =>
				renderSnippet(
					createRawSnippet(() => ({
						render: () => '<div class="w-full text-end">제한</div>',
					}))
				),
...
<script lang="ts">
	...
	let viewLabel = $derived(views.find((v) => view === v.id)?.label ?? "보기 선택");
</script>

<Tabs.Root value="outline" class="w-full flex-col justify-start gap-6">
	<div class="flex items-center justify-between px-4 lg:px-6">
		<Label for="view-selector" class="sr-only">보기</Label>
...
{#snippet DataTableLimit({ row }: { row: Row<Schema> })}
	...
	>
		<Label for="{row.original.id}-limit" class="sr-only">제한</Label>
		<Input
...
{#snippet DataTableActions()}
	<DropdownMenu.Root>
		...
		<DropdownMenu.Content align="end" class="w-32">
			...
			<DropdownMenu.Item>사본 만들기</DropdownMenu.Item>
